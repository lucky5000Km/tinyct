...
unix:!macx: {
    ...
    desktop.files = deploy/linux/com.tinyct.TinyCt.desktop
    desktop.path = $$PREFIX/$$DATADIR/applications/

    icons.files = res/tinyct.svg
    icons.path = $$PREFIX/$$DATADIR/icons/hicolor/scalable/apps/

    appstream.files = deploy/linux/com.tinyct.TinyCt.appdata.xml
...
win32 {
    RC_ICONS = tinyct.ico
    QMAKE_TARGET_COMPANY = TinyCt Project
    QMAKE_TARGET_DESCRIPTION = TinyCt Game Streaming Client
    QMAKE_TARGET_PRODUCT = TinyCt

    CONFIG -= embed_manifest_exe
    QMAKE_LFLAGS += /MANIFEST:embed /MANIFESTINPUT:$${PWD}/tinyct.exe.manifest
}
macx {
    ...
    APP_BUNDLE_RESOURCES.files = tinyct.icns
