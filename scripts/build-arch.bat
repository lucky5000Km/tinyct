...
echo Compiling tinyct in %BUILD_CONFIG% configuration
pushd %BUILD_FOLDER%
...
if !ERRORLEVEL! NEQ 0 goto Error
7z a %SYMBOLS_FOLDER%\TinyCtDebuggingSymbols-%ARCH%-%VERSION%.zip %SYMBOLS_FOLDER%\*.pdb
if !ERRORLEVEL! NEQ 0 goto Error

if "%ML_SYMBOL_STORE%" NEQ "" (
    echo Publishing PDBs to symbol store: %ML_SYMBOL_STORE%
    symstore add /f %SYMBOLS_FOLDER%\*.pdb /s %ML_SYMBOL_STORE% /t tinyct
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
...
if "%ML_SYMBOL_ARCHIVE%" NEQ "" (
    ...
    copy %SYMBOLS_FOLDER%\TinyCtDebuggingSymbols-%ARCH%-%VERSION%.zip %ML_SYMBOL_ARCHIVE%
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
    if "%MUST_DEPLOY_SYMBOLS%"=="1" (
...
%WINDEPLOYQT_CMD% --dir %DEPLOY_FOLDER% --%BUILD_CONFIG% --qmldir %SOURCE_ROOT%\app\gui --no-opengl-sw --no-compiler-runtime --no-sql %WINDEPLOYQT_ARGS% %BUILD_FOLDER%\app\%BUILD_CONFIG%\tinyct.exe
if !ERRORLEVEL! NEQ 0 goto Error
...
if "%SIGN%"=="1" (
    ...
    set FILES_TO_SIGN=%BUILD_FOLDER%\app\%BUILD_CONFIG%\tinyct.exe
...
if "%ML_SYMBOL_STORE%" NEQ "" (
    ...
    symstore add /r /f %DEPLOY_FOLDER%\*.* /s %ML_SYMBOL_STORE% /t tinyct
    if !ERRORLEVEL! NEQ 0 goto Error
    symstore add /r /f %BUILD_FOLDER%\app\%BUILD_CONFIG%\tinyct.exe /s %ML_SYMBOL_STORE% /t tinyct
    if !ERRORLEVEL! NEQ 0 goto Error
)

echo Building MSI
cmd /c "set VERSION= && msbuild -Restore %SOURCE_ROOT%\wix\tinyct\tinyct.wixproj /p:Configuration=%BUILD_CONFIG% /p:Platform=%ARCH% /p:MSBuildProjectExtensionsPath=%BUILD_FOLDER%\"
if !ERRORLEVEL! NEQ 0 goto Error

echo Copying application binary to deployment directory
copy %BUILD_FOLDER%\app\%BUILD_CONFIG%\tinyct.exe %DEPLOY_FOLDER%
if !ERRORLEVEL! NEQ 0 goto Error
...
if defined CI_VERSION (
    ...
    rem This file tells tinyct that it's a portable installation
    echo. > %DEPLOY_FOLDER%\portable.dat
    if !ERRORLEVEL! NEQ 0 goto Error
)

7z a %INSTALLER_FOLDER%\tinyctPortable-%ARCH%-%VERSION%.zip %DEPLOY_FOLDER%\*
if !ERRORLEVEL! NEQ 0 goto Error

echo Build successful for tinyct v%VERSION% %ARCH% binaries!
exit /b 0
